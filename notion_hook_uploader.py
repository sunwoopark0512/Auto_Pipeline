...
def parse_generated_text(text):
    """Parse JSON text generated by GPT and return sanitized fields."""
    try:
        data = json.loads(text)
    except json.JSONDecodeError:
        logging.warning("⚠️ JSON 디코딩 실패")
        return {
            "hook_lines": ["", ""],
            "blog_paragraphs": ["", "", ""],
            "video_titles": ["", ""]
    ...
    hook_lines = data.get("hook_lines") or []
    blog_paragraphs = data.get("blog_paragraphs") or []
    video_titles = data.get("video_titles") or []

    return {
        "hook_lines": [str(h) for h in hook_lines][:2] + ["", ""][: max(0, 2 - len(hook_lines))],
        "blog_paragraphs": [str(p) for p in blog_paragraphs][:3] + ["", "", ""][: max(0, 3 - len(blog_paragraphs))],
        "video_titles": [str(v) for v in video_titles][:2] + ["", ""][: max(0, 2 - len(video_titles))]
    }
