...
def _dynamic_import(module_name: str) -> ModuleType:
    ...
    for mod, seen in _LOADED_PATHS.items():
        if seen == real_path and mod != module_name:
            raise ImportError(f"Duplicate module path detected: {module_name} conflicts with {mod}")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Import steps without executing them")
