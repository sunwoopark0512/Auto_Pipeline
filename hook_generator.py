...
async def generate_hooks():
    ...
    async def process_keyword(item):
    ...
    for item in keywords:
        ...
        tasks.append(asyncio.create_task(process_keyword(item)))
        await asyncio.sleep(API_DELAY)

    for status, result in await asyncio.gather(*tasks):
        if status == "success":
            new_output.append(result)
            logging.info(f"✅ 생성 완료: {result['keyword']}")
            success += 1
        else:
            failed_output.append(result)
            logging.error(f"❌ 생성 실패: {result['keyword']}")
            failed += 1
