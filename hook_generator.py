...
# ---------------------- GPT 호출 함수 (재시도 포함) ----------------------
async def get_gpt_response(prompt, semaphore, retries=3):
...
async def process_keyword(item, semaphore):
    ...
    response = await get_gpt_response(prompt, semaphore)
...
async def generate_hooks():
    ...
    for item in keywords:
        ...
        tasks.append(asyncio.create_task(process_keyword(item, semaphore)))
